        ...
        <section id="mainView">
            ...
            <div class="container">
                    ...
                    <table>
                        <thead>
                            <?php
                                ...
                                $sql = "SELECT b.planTitle ,b.planContent,b.planGps, b.boardView, b.boardLike, b.regTime, b.myMemberID, m.youName, m.youScore, m.youImg, m.youIntro FROM gPlanBoard b JOIN gmember m ON (b.myMemberID = m.myMemberID) WHERE b.myBoardID = ${boardID}";
                                $result = $connect -> query($sql);
                                $info = $result -> fetch_array(MYSQLI_ASSOC);
                            ?>
   
                            <?php 
                                $view = "UPDATE gPlanBoard SET boardView = boardView + 1 WHERE myBoardID = {$boardID}";
                ...
                <div class="fViewText">  
                    ...
                    <div class="commBoardView">
                        <?php 
                            $noShow = true;
                            $_GET['title'] = $info['planTitle']; 
                            $_GET['planContent'] = $info['planContent'];
                            $_GET['gps'] = $info['planGps'];
                            include '../schedule/show.php'
                        ?>
                    </div>
                    <?php
                        if( isset( $_SESSION['myMemberID'] ) ){
                            echo "<a class='commentBtn dilike board' href='./commBoardLike.php?like=dislike&memberID=".$info['myMemberID']."&boardID=".$boardID."'>비추천</a>";
                            echo "<a class='commentBtn like board' href='./commBoardLike.php?like=like&memberID=".$info['myMemberID']."&boardID=".$boardID."''>추천</a>";
                        }
                    ?>
                        ...
                        <?php
                            ...
                            */
                            if( !isset($_SESSION['myMemberID']) ){
                                ...
                                echo "<textarea name='commentText2' id='commentText' class='commentText' rows='4'  placeholder='댓글 내용을 작성해 주세요!' required></textarea>";
                ...
                <section class="comment">
                    <div class="commentwrap">
                        <?php
                            $boardID = $_GET['boardID'];
                            $sql = "SELECT c.myCommentID, c.myMemberID, c.youText, c.youLike, c.youDislike, c.regTime, m.youName FROM planComment c JOIN gmember m ON (c.myMemberID = m.myMemberID) WHERE c.myBoardID = ${boardID}";
                            $result = $connect -> query($sql);
                            if($result){
                                ...
                                if($count > 0){
                                    for($i=1; $i<=$count;$i++){
                                        ...
                                        if( isset( $_SESSION['myMemberID'] ) ){
                                            echo "<a href='../comment/commentLike.php?like=dislike&memberID=".$info['myMemberID']."&commentID=".$info['myCommentID']."&plane=plane' class='commentBtn dilike'>비추천</a>";
                                            echo "<a href='../comment/commentLike.php?like=like&memberID=".$info['myMemberID']."&commentID=".$info['myCommentID']."&plane=plane' class='commentBtn like'>추천</a>";
                                        }
                                        ...
                                        echo "<div class='commentInfo'>";
                                            ...
                                            if( isset($_SESSION['myMemberID']) ){
                                                echo "<a href='../comment/commentRemove.php?commentID=".$info['myCommentID']."&plane=plane' class='comRemoveBtn'>삭제하기</a>";
                                            }
                                        echo "</div>";
                                    }
                                }
        ...
        if( cf ){
            cf.addEventListener("click",function(e){
                if( confirm("정말로 해당 글을 삭제 하시겠습니까?") == true ){
                    ...
                    window.location = "./commBoardRemove.php?"+queryString[1];
                } else {
                    console.log("취소");
                }
            });
